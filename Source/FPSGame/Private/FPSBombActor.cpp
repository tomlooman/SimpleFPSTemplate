...
void AFPSBombActor::Explode()
{
	...
	for (FOverlapResult Result : OutOverlaps)
	{
		...
		if (Overlap && Overlap->IsSimulatingPhysics())
		{
			...
			// Bonus Code
			FVector Direction = GetActorLocation() - Overlap->GetComponentLocation();
			Direction.Normalize();
			Overlap->AddRadialImpulse(GetActorLocation(), 500.0f, 1000.0f, ERadialImpulseFalloff::RIF_Constant, true);
		}
	}
